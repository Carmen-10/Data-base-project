...
# Definește coloana “ownerId” drept cheie primară a tabelei “owners”
ALTER TABLE owners
...
# Numară vehiculele inregistrate, grupate pe categorii de autovehicule
SELECT COUNT(vehicleId), vehicleType FROM vehicles GROUP BY vehicleType;
...
SELECT SUM(taxValueOfCurrentYear) FROM tax
WHERE dayOfPayment IS NOT NULL
and extract(year from dayOfPayment) = extract(year from current_date);

# Afișează, pe categorii de autovehicule, sumele încasate din taxele achitate pentru anul curent
SELECT SUM(taxValueOfCurrentYear), vehicles.vehicleType FROM tax 
LEFT JOIN vehicles ON tax.vehicleId = vehicles.vehicleId
WHERE dayOfPayment IS NOT NULL
...
# Afiseaza proprietarii si autovehiculele pentru care acestia si-au platit impozitele
SELECT owners.firstName, owners.lastName, vehicles.vehicleModel, tax.dayOfPayment FROM owners
JOIN tax ON owners.ownerId = tax.ownerId
JOIN vehicles ON vehicles.vehicleId = tax.vehicleId
WHERE dayOfPayment IS NOT NULL;

# Afiseaza tipurile de vehicule inregistrate
SELECT DISTINCT vehicleType FROM vehicles;

# Afiseaza cate categorii de vehicule sunt inregistrate
SELECT COUNT(DISTINCT vehicleType) FROM vehicles;

# Sterge tabela "owners'/ "vehicles"/ "tax" 
DROP table owners;
DROP table vehicles;
DROP table tax;
...
# Alternativ - sterge baza de date "vehicles_tax"
DROP DATABASE vehicles_tax;
